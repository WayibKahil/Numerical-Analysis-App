...
class GaussJordanPartialPivotingMethod(NumericalMethodBase):
    def solve(self, matrix_str: str, vector_str: str, decimal_places: int = 6) -> Tuple[List[float], List[Dict]]:
        """
        Solve a system of linear equations using the Gauss-Jordan method with partial pivoting.
        This method transforms the augmented matrix to reduced row echelon form (identity matrix)
        while using partial pivoting for better numerical stability.
        """
        try:
            ...
            for i in range(n):
                ...
                if pivot_row != i:
                    ...
                    row = {
                        ...
                        "Operation": f"Swap R{i+1} with R{pivot_row+1} because |{self._format_value(augmented[i, i], decimal_places)}| > |{self._format_value(A[i, i], decimal_places)}|"
                    }
                    table.append(row)
            ...
            return None, [{"Error": f"Error in Gauss-Jordan with Partial Pivoting method: {str(e)}"}]
